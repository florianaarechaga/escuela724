...
function education_zone_save_sidebar_layout( $post_id ) { 
    global $education_zone_sidebar_layout, $post; 
    ...
    foreach( $education_zone_sidebar_layout as $field ){  
        ...
        $old = get_post_meta( $post_id, 'education_zone_sidebar_layout', true); 
        $new = sanitize_text_field( $_POST['education_zone_sidebar_layout'] );
        if ( $new && $new != $old ) {  
            update_post_meta( $post_id, 'education_zone_sidebar_layout', $new );  
        } elseif ( '' == $new && $old ) {  
            delete_post_meta( $post_id,'education_zone_sidebar_layout', $old );  
        }  
     } // end foreach   
